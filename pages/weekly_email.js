  ...
  return <Email title="Stats Check Lol Weekly Summary!" headCSS={css} style={{ width: '100%', maxWidth: '600px' }}>
    ...
    <Box>
      ...
      {['Likes', 'Shares', 'Comments', 'Views'].map(listType =>
        <><Spacer /><BestList listType={listType} key={listType} /></>)}
        <Spacer />
      <Item align="center">
...
function Glances() {
  return <BorderedBox children={<Item><ul style={{paddingLeft: '20px'}}>
    ...
    ['trending', <span>Your most-viewed video created in the last week was <a href="https://www.tiktok.com/{{tiktok_username}}/video/{{most_popular_video_id}}">{'{{'}most_popular_video_caption{'}}'}</a>. It had <strong>{'{{'}most_popular_video_views{'}}'}</strong> total views. (Keep in mind that many of your views may come from videos created more than one week ago.)</span>],
  ...
  {'{{/if}}'}</span>],
      ...
      <GlanceElement icon={v[0]} text={v[1]} key={v[1]}/>)}
  </ul></Item>}
    header='At a Glance' />
}
  ...
  return <BorderedBox
    ...
    children={<>
      ...
      <tr>
        ...
        <td height="10" class="em_h15" style={{ paddingLeft: '5px', borderTop: 'solid 1px #bbb' }}>
