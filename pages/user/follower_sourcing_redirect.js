...
export default function User( { error }) {  
    ...
    if(error == 'not_pro') { return <Error notPro/> }
...
export async function getServerSideProps({ params , req, res}) {
    ...
    if(!session?.pro) return { props: { error: 'not_pro'}}
    const authed = await auth(session, res);
    if(authed.error) {return { props: { error: authed.error }}}
...
function redirect(res, tiktok_username) {
    if (res) {
      res.writeHead(302, {
        Location: '/user/' + tiktok_username.toLowerCase() + '/follower_sourcing'
      });
      res.end();
    }  
}
